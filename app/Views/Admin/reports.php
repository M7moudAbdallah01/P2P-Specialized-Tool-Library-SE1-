...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adjust_trust'])) {
    ...
    $delta      = intval($_POST['trust_delta']);
    $conn->query("
        UPDATE users
    ...
    $conn->query("UPDATE repair_requests SET status='resolved' WHERE id=$dmg_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_repair'])) {
    ...
    $conn->query("
        ...
        SET technician_id=$tech_id, admin_note='$adm_note', status='reviewing', updated_at=NOW()
        WHERE id=$dmg_id
    ");
...
/* =========================================================
   FETCH DAMAGE / REPAIR REQUESTS
========================================================= */
$dmg_where = "WHERE 1=1";
if ($status_filter && in_array($status_filter, ['pending','reviewing','resolved'])) {
    $dmg_where .= " AND rr.status='$status_filter'";
}
if ($search) {
    $s = $conn->real_escape_string($search);
    $dmg_where .= " AND (rr.tool_name LIKE '%$s%' OR rr.reservation_id LIKE '%$s%' OR u.name LIKE '%$s%')";
}
...
/* ── Technicians list ── */
$techs_q     = $conn->query("SELECT user_id, name FROM users WHERE role='technical' ORDER BY name");
$technicians = [];
...
.dc-detail{display:none;padding:16px 20px;border-top:1px solid var(--border,#2a2a2a);background:#161616;}
    ...
    <div class="main-content">
        ...
        <div class="alert alert-success">
            ...
            <?= match($_GET['msg']) {
                ...
                'repair_sent'   => 'Repair request assigned to technician.',
            ...
            <div class="damage-card <?= $stat_cls ?>" id="dcard-<?= $dd['id'] ?>">
                ...
                <div class="dc-body">
                    ...
                    <div class="dc-col">
                        ...
                        <?php if ($dd['technician_id']): ?>
                        <div class="dc-col-label" style="margin-top:14px;">Assigned Technician</div>
                        <div class="dc-col-val" style="color:#6366f1;">
                            <i class="fa fa-screwdriver-wrench"></i>
                            <?php
                                $tq = $conn->query("SELECT name FROM users WHERE user_id={$dd['technician_id']}");
                                echo $tq ? htmlspecialchars($tq->fetch_assoc()['name'] ?? '—') : '—';
                            ?>
                        </div>
                        <?php if ($dd['admin_note']): ?>
                        <div class="dc-col-sub"><?= htmlspecialchars($dd['admin_note']) ?></div>
                        <?php endif; ?>
                        <?php endif; ?>
                    </div>
                ...
                <div class="dc-foot">
                    ...
                    <div style="font-size:.8rem;color:#6366f1;padding-left:14px;">
                        <i class="fa fa-circle-info"></i> Already assigned to a technician.
                    </div>
                    <?php endif; ?>
